#!/usr/bin/env python
# md5: 03265d90a3e210ae589bc13f84b2224e
# coding: utf-8
...
if n_enabled:
  num_labels_enabled = 2 + num_prev_enabled # since we disabled the n label
else:
  num_labels_enabled = 1 + num_prev_enabled # since we disabled the n label
data_version = 4+8*(vernum-2) + num_prev_enabled
...
def get_row_names(include_domain=False, printing=False):
  if printing:
    loc_prev_enabled = 7
  else:
    loc_prev_enabled = num_prev_enabled
  output_row_names = [
    'label',
    'spanlen',
    'since_cur',
  ...
  for idx_p_zeroidx in range(loc_prev_enabled):
...
#print 'output.append([' + ', '.join([get_row_names(True, True)]) + '])'


print 'cached_locals = {' + ', '.join(['"' + x + '": ' + x for x in get_row_names(True, True)]) + '}'
...
def get_rows_for_user(user, include_domain=False):
  ...
  domain_id_to_history_idx = Counter()
  for idx,visit in enumerate(ordered_visits):
    if idx+1 >= len(ordered_visits):
      break
    
    next_visit = ordered_visits[idx+1]
    referring_visit_id = int(next_visit['referringVisitId'])
    cur_referring_visit_id = int(visit['referringVisitId'])
    nref_eq_zero = 'T' if referring_visit_id == 0 else 'F'
    cref_eq_zero = 'T' if cur_referring_visit_id == 0 else 'F'
    ...
    domain_id_to_history_idx[cur_domain_id] = idx
    
    visit_id_to_domain_id[int(visit['visitId'])] = cur_domain_id
    nref_domain_id = visit_id_to_domain_id.get(referring_visit_id, -1)
    cref_domain_id = visit_id_to_domain_id.get(cur_referring_visit_id, -1)
    ...
    c_eq_p1 = 'T' if (cur_domain_id == prev1_domain_id) else 'F'
    c_eq_p2 = 'T' if (cur_domain_id == prev2_domain_id) else 'F'
    c_eq_p3 = 'T' if (cur_domain_id == prev3_domain_id) else 'F'
    c_eq_p4 = 'T' if (cur_domain_id == prev4_domain_id) else 'F'
    c_eq_p5 = 'T' if (cur_domain_id == prev5_domain_id) else 'F'
    c_eq_p6 = 'T' if (cur_domain_id == prev6_domain_id) else 'F'
    c_eq_p7 = 'T' if (cur_domain_id == prev7_domain_id) else 'F'
    ...
    cref_eq_n = 'T' if cref_domain_id == next_domain_id else 'F'
    cref_eq_c = 'T' if cref_domain_id == cur_domain_id else 'F'
    cref_eq_p1 = 'T' if cref_domain_id == prev1_domain_id else 'F'
    cref_eq_p2 = 'T' if cref_domain_id == prev2_domain_id else 'F'
    cref_eq_p3 = 'T' if cref_domain_id == prev3_domain_id else 'F'
    cref_eq_p4 = 'T' if cref_domain_id == prev4_domain_id else 'F'
    cref_eq_p5 = 'T' if cref_domain_id == prev5_domain_id else 'F'
    cref_eq_p6 = 'T' if cref_domain_id == prev6_domain_id else 'F'
    cref_eq_p7 = 'T' if cref_domain_id == prev7_domain_id else 'F'
    
    visits_since_p1 = idx - domain_id_to_history_idx.get(prev1_domain_id, 0)
    visits_since_p2 = idx - domain_id_to_history_idx.get(prev2_domain_id, 0)
    visits_since_p3 = idx - domain_id_to_history_idx.get(prev3_domain_id, 0)
    visits_since_p4 = idx - domain_id_to_history_idx.get(prev4_domain_id, 0)
    visits_since_p5 = idx - domain_id_to_history_idx.get(prev5_domain_id, 0)
    visits_since_p6 = idx - domain_id_to_history_idx.get(prev6_domain_id, 0)
    visits_since_p7 = idx - domain_id_to_history_idx.get(prev7_domain_id, 0)
    ...
    for time_sec in xrange(cur_time_sec+1, next_time_sec):
      ...
      # c p n p q r s t
      if immediate_prev:
        available_labels = [(x,y.replace('p', 'i')) for x,y in available_labels]
      ...
      cached_locals = {"time_sec": time_sec, "user": user, "ref_domain": ref_domain, "label": label, "spanlen": spanlen, "since_cur": since_cur, "cur_domain_letter": cur_domain_letter, "to_next": to_next, "next_domain_letter": next_domain_letter, "n_eq_c": n_eq_c, "nref_eq_c": nref_eq_c, "nref_eq_zero": nref_eq_zero, "cref_eq_c": cref_eq_c, "cref_eq_n": cref_eq_n, "cref_eq_zero": cref_eq_zero, "since_prev1": since_prev1, "prev1_domain_letter": prev1_domain_letter, "n_eq_p1": n_eq_p1, "c_eq_p1": c_eq_p1, "nref_eq_p1": nref_eq_p1, "cref_eq_p1": cref_eq_p1, "visits_since_p1": visits_since_p1, "since_prev2": since_prev2, "prev2_domain_letter": prev2_domain_letter, "n_eq_p2": n_eq_p2, "c_eq_p2": c_eq_p2, "nref_eq_p2": nref_eq_p2, "cref_eq_p2": cref_eq_p2, "visits_since_p2": visits_since_p2, "since_prev3": since_prev3, "prev3_domain_letter": prev3_domain_letter, "n_eq_p3": n_eq_p3, "c_eq_p3": c_eq_p3, "nref_eq_p3": nref_eq_p3, "cref_eq_p3": cref_eq_p3, "visits_since_p3": visits_since_p3, "since_prev4": since_prev4, "prev4_domain_letter": prev4_domain_letter, "n_eq_p4": n_eq_p4, "c_eq_p4": c_eq_p4, "nref_eq_p4": nref_eq_p4, "cref_eq_p4": cref_eq_p4, "visits_since_p4": visits_since_p4, "since_prev5": since_prev5, "prev5_domain_letter": prev5_domain_letter, "n_eq_p5": n_eq_p5, "c_eq_p5": c_eq_p5, "nref_eq_p5": nref_eq_p5, "cref_eq_p5": cref_eq_p5, "visits_since_p5": visits_since_p5, "since_prev6": since_prev6, "prev6_domain_letter": prev6_domain_letter, "n_eq_p6": n_eq_p6, "c_eq_p6": c_eq_p6, "nref_eq_p6": nref_eq_p6, "cref_eq_p6": cref_eq_p6, "visits_since_p6": visits_since_p6, "since_prev7": since_prev7, "prev7_domain_letter": prev7_domain_letter, "n_eq_p7": n_eq_p7, "c_eq_p7": c_eq_p7, "nref_eq_p7": nref_eq_p7, "cref_eq_p7": cref_eq_p7, "visits_since_p7": visits_since_p7, "switchto_in_session_cur": switchto_in_session_cur, "switchto_in_session_next": switchto_in_session_next, "switchto_in_session_prev1": switchto_in_session_prev1, "switchto_in_session_prev2": switchto_in_session_prev2, "switchto_in_session_prev3": switchto_in_session_prev3, "switchto_in_session_prev4": switchto_in_session_prev4, "switchto_in_session_prev5": switchto_in_session_prev5, "switchto_in_session_prev6": switchto_in_session_prev6, "switchto_in_session_prev7": switchto_in_session_prev7}
